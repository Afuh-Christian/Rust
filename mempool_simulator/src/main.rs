...
async fn main() {
    ...
    // --- Producer Tasks (Parallel Load Generation) ---
    // We spawn 4 producers to saturate the mempool's concurrent capabilities
    for i in 0..4 {
        let mempool_producer = mempool.clone();
        tokio::spawn(async move {
            let mut nonce = (i as u64) * 10_000_000; // Unique nonce ranges
            loop {
                // 1. Generate batch in parallel (CPU heavy)
                let batch: Vec<_> = (nonce..nonce + 1000)
                    .into_par_iter()
                    .map(|n| generate_tx(n))
                    .collect();

                // 2. Insert batch (Lock heavy)
                for tx in &batch {
    ...
    tokio::spawn(async move {
        loop {
            ...
            // Mine approx 500 txs (21k gas each)
